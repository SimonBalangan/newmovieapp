...
import Trailer from '../About/Trailer';
...
const SciFi = () => {
  ...
  const getHome = useCallback(async () => {
    ...
    try {
        const response = await client.getEntry('6RH5JzoA3LwNqMLVTRmviI')
  ...
  return (
    ...
    <CardGroup className='cardGroup md'> 

      <Card className="bg-dark text-white card-link">
        <Card.Img className='cardImg' variant="top" src={homeImage1}  />
          
        <Card.Body>
          <Card.Title className='cardTitle'><p>{homeTitel1}</p></Card.Title>
          <Info/>
        </Card.Body>
